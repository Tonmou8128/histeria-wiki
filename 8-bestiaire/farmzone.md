%%name=Farmzone%%
%%icon=https://raw.githubusercontent.com/HisteriaMC/histeria-wiki/main/.assets/icons/events/farmzone-icon.png%%
%%weight=100%%

# Farmzone

## Explications

La farmzone est un serveur dans lequel il est possible de tuer des créatures pour avoir un meilleur butin qu'avec les spawners. Ici, chaque créature peut donner son butin sous une forme rare, la forme "primordiale".

Accessible via `/farmzone`, elle propose différents biomes avec chacun sa créature dominante. En farmzone, les créatures sont agressives et ont plus de vie. Il faut donc être préparé pour y aller.

Mais de mystérieux marchands s'y trouvent également. Ils proposent différents échanges.

## Hermite

Ce marchand se situe au fond des marais, dans une cabane en bois. Il propose, entre 2 légendes locales, d'échanger de l'argent, du lumen et des objets de créatures contre des nouveaux objets uniques !

| Objet | Fonctionnement | Argent | Lumen | Objets de créature |
| --- | --- | --- | --- | --- |
| Poudre de fusion | Améliore le % de réussite d'un parchemin | 100 $ | 50 | 4 cornes d'Histerite |
| Kukri sanguin | Un shuriken qui inflige des dégâts sur la durée | 1 000 $ | 500 | 4 griffes sylvestres et 4 fourrures dorées |
| Boule de lumen | Immobilise l'ennemi | 1 500 $ | 750 | 16 larmes de lumen |
| Ender pearl divisée | Une perle de l'end qui divise en l'air | 2 500 $ | 1 250 | 16 cornes d'Histerite et 16 griffes sylverstres |
| Potion de purifiaction | Retire les effets négatifs | 10 000 $ | 5 000 | 32 larmes de lumen et 2 larmes primordiales |
| Bouclier d'objet | Alerte lorsque l'objet va se casser | 15 000 $ | 7 500 | 32 filaments nocturnes et 2 filaments primordiaux |
| Marqueur de joueur | Met le nom du joueur sur un objet | 20 000 $ | 10 000 | 4 griffes primordiales |
| Marqueur d'éliminations | Compte le nombre d'élimination par un objet | 20 000 $ | 10 000 | 4 cornes primordiales |
| Marqueur de blocs | Compte le nombre de blocs minés par un objet | 20 000 $ | 10 000 | 4 fourrures primordiales |
| Codex d'enchantement | Améliore un enchantement au niveau max | 50 000 $ | 25 000 | 16 filaments primordiaux et 16 griffes primordiales |
| Enchantement Gloom I | Invoque un Hylide sur l'adversaire | 50 000 $ | 27 500 | 32 larmes primordiales |
| Casque de lumen | Avec l'armure entière, donne des boosts pour tuer des créatures | 150 000 $ | 100 000 | 32 larmes primordiales et 32 cornes primordiales |
| Bottes de lumen | Avec l'armure entière, donne des boosts pour tuer des créatures | 150 000 $ | 100 000 | 32 larmes primordiales et 32 filaments primordiaux |
| Jambières de lumen | Avec l'armure entière, donne des boosts pour tuer des créatures | 225 000 $ | 150 000 | 48 larmes primordiales et 48 griffes primordiales |
| Plastron de lumen | Avec l'armure entière, donne des boosts pour tuer des créatures | 300 000 $ | 200 000 | 64 larmes primordiales et 64 fourrures primordiales |
| Epée de lumen | Enchantée Verdance I (donne plus de lumen en tuant des créatures) | 400 000 $ | 250 000 | 64 cornes primordiales et 64 griffes primordiales |

## Hérétique

Ce marchand échange 50 polynectars contre 3 ou 4 enchantements d'un coup sur un objet. Il se situe dans une tente sur le territoire des Lycos.
